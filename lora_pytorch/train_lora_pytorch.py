...
# 导入自定义模块
from GPT2_pytorch import GPT
from LoRA_pytorch import LoRAConfig, check_grads, verify_and_fix_lora_weights
from lora_models_pytorch import LoRAGPT
from model_utils_pytorch import SQuADDataLoader, evaluate, generate_answer
...
def get_performance_metrics():
    ...
    try:
        if torch.cuda.is_available():
            ...
            metrics['gpu_memory_reserved_gb'] = torch.cuda.memory_reserved(0) / (1024 ** 3)
        else:
...
def train_default_lora(
    ...
    output_dir='./checkpoints/default_lora_pytorch',
    lora_r=8,
    lora_alpha=16,
    lora_dropout=0.1,
...
# 评估模型
def evaluate_model(model, tokenizer, test_samples=None, logger=None):
    """
    评估模型在一些示例上的表现
...
def load_and_evaluate_model(
    base_model_path='./local_gpt2',
    lora_weights_path='./checkpoints/default_lora_pytorch/gpt2_lora_final.npz',
    lora_r=8,
    lora_alpha=16,
    lora_dropout=0.1,
    target_modules=["c_attn", "c_proj", "c_fc"],
    ...
    timestamp = datetime.datetime.now().strftime("%Y%m%d_%H%M%S")
    test_log_file = os.path.join(log_dir, f"evaluation_{timestamp}.log")
    test_logger = setup_logger('evaluation_logger', test_log_file)
    ...
    test_logger.info(f"评估配置: {json.dumps(config, ensure_ascii=False, indent=2)}")
    ...
    # 评估模型
    results = evaluate_model(model, tokenizer, test_samples, logger=test_logger)
    ...
    test_logger.info(f"评估结果已保存至: {results_path}")
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="训练或评估LoRA模型")
    parser.add_argument("--mode", type=str, choices=["train", "eval"], default="train", help="运行模式：训练或评估")
    parser.add_argument("--base_model", type=str, default="./local_gpt2", help="基础模型路径")
    parser.add_argument("--data_path", type=str, default="./data/processed_squad_100.json", help="训练数据路径")
    parser.add_argument("--output_dir", type=str, default="./checkpoints/default_lora_pytorch", help="输出目录")
    ...
    parser.add_argument("--lora_weights", type=str, default=None, help="LoRA权重文件路径（评估模式下使用）")
    ...
    if args.mode == "train":
        ...
        # 评估模型
        eval_results = evaluate_model(model, tokenizer, logger=logger)
        ...
        logger.info("\n模型训练和评估完成!")
        logger.info(f"评估结果已保存至: {eval_results_path}")
        logger.info("可以通过以下命令加载模型并进行评估:")
        logger.info(f"python train_lora_pytorch.py --mode eval --lora_weights {os.path.join(args.output_dir, 'gpt2_lora_final.npz')}")
        ...
        # 加载模型并评估
        results, logger = load_and_evaluate_model(
            base_model_path=args.base_model,
            lora_weights_path=lora_weights_path,
            lora_r=args.lora_r,
            lora_alpha=args.lora_alpha,
