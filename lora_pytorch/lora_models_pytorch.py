...
from GPT2_pytorch import Block, CausalSelfAttention, MLP
from LoRA_pytorch import LoRALinear, LoRAQKVProjection, LoRAConfig
...
class LoRACausalSelfAttention(nn.Module):
    ...
    def forward(self, x):
        ...
        # 计算注意力分数
        att = (q @ k.transpose(-2, -1)) * (1.0 / torch.sqrt(torch.tensor(self.head_size, dtype=torch.float32)))
...
class LoRAGPT(nn.Module):
    ...
    def save_lora_weights(self, save_path):
        ...
        for name, param in self.named_parameters():
            if 'lora_A' in name or 'lora_B' in name:
                lora_state_dict[name] = param.detach().cpu().numpy()
